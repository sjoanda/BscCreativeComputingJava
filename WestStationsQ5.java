...
private static void BubbleSort(double[] StationLongitude) 
{
	for (int i = 0; i < StationLongitude.length; i++) 
		{
		for (int x = 1; x < StationLongitude.length - i; x++) 
			{
			if (StationLongitude[x - 1] > StationLongitude[x]) 
				{
            double temp             = StationLongitude[x - 1];
            StationLongitude[x - 1] = StationLongitude[x];
            StationLongitude[x]     = temp;
				
				/* Money for old rope, this... */
   ...
   public static void main(String[] args) throws Exception
   {
      ...
      Scanner MyInputStream = new Scanner(new FileReader("stations")); 
	   ...
	   while (MyInputStream.hasNext())
		...
		BubbleSort(StationLongitude);
	
		/* Now print out the results */
		for (RecordNumber = 0; RecordNumber < NumberOfRecordsRead; ++RecordNumber)
			System.out.println("Station " + StationName[RecordNumber] + "(" + StationIdentityNumber[RecordNumber] + "), Long = " + StationLongitude[RecordNumber]);
   
		/* The spec says that out required station has n + 1 stations to the east than 
         there are to the west. So, we want the 
		*/	
	   int RequiredStationNumber = (NumberOfRecordsRead / 2) - 1;
		System.out.println("The required station is " + StationName[RequiredStationNumber] + " (" + StationIdentityNumber[RequiredStationNumber] + ") Longitude = " + StationLongitude[RequiredStationNumber]);
	}
}
