/* Program name			weightAdjacentStationsQ4
	Version					01-00
	...
	This program takes two stations input as integer values, and finds the distance between them as the crow flies.  This is identical to Q3 asides from a change of the signs < and >
	------------------------------------------------------------------------------
...
public class weightAdjacentStationsQ4
{
...
/*         int MyRecordNumber = 0;
      ...
      MyInputStream = new Scanner(new FileReader("edges")); 
		...
		*/
		...
		double MyClosestConnectedStationDistance = 10; //furthest is 8km apart
		int StationOneClosest = 0;
		int StationTwoClosest = 0;
	   while (MyInputStream.hasNext())
	      {
			/* Build the matrix */
		   MyInputString = MyInputStream.nextLine();
         ...
         double InterstationDistance = HaversineFormula(StationOneLatitude, StationOneLongitude, StationTwoLatitude, StationTwoLongitude);
	      ...
	      edges[StationOne][StationTwo] = InterstationDistance;
		   edges[StationTwo][StationOne] = InterstationDistance;
			...
			if (InterstationDistance < MyClosestConnectedStationDistance)
				{
				MyClosestConnectedStationDistance = InterstationDistance;
				StationOneClosest = StationOneRecordNumber;
				StationTwoClosest = StationTwoRecordNumber;
				}
			NumberOfEdgesRead++;
	      }
	
   int StationOneID = StationIdentityNumber[StationOneClosest];
   int StationTwoID = StationIdentityNumber[StationTwoClosest];	
	double InterstationDistance = edges[StationOneID][StationTwoID];
	System.out.println("The stations which are closest together are: Station 1 " + StationName[StationOneClosest] + "(" + StationIdentityNumber[StationOneClosest] + ") and " +  StationName[StationTwoClosest] + "(" + StationIdentityNumber[StationTwoClosest] + ") : Distance " + Math.round(InterstationDistance * 1000) + " m");		
   }
}
